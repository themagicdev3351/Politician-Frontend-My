...
const YourUploaded = () => {
  ...
  const handleOptionClick = (userId) => {
    ...
    navigate("/SingleUserPreview", { state: { active: { id: adminId, status: true } } });
  };
  ...
  const DeletePost = async () => {
    ...
    try {
      ...
      actionDispatcher(openSnackbar("Study material deleted Successfully", "success"));
  ...
  const GetAllStudyMaterial = async () => {
    try {
      const res = await apiService.GetyourUploadeMaterial();
      console.log("res uploaded study====>", res);
      setData(res?.studyMaterials);
    } catch (error) {
      if (error.response) {
    ...
    >
      {data && data?.length > 0 ? (
        ...
        <MDBox sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
          <MDTypography>Not Uploaded Yet.</MDTypography>
        </MDBox>
      )}
    </MDBox>
  );
