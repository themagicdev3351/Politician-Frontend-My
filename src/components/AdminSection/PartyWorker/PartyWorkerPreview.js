...
const PartyWorkerPreview = () => {
    const { state } = useLocation();
    const [data, setData] = useState({});
    const navigate = useNavigate();
    console.log("user id", state.active.id)
    const getUser = async () => {
        try {
            const res = await apiService?.getSingleUser(state.active.id);
            console.log("party worker", res)
                    ...
                    <Stack spacing={1} mt={2}>
                        ...
                        <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                            ...
                            {data.userType === "mentor" ? (
                                <Box sx={{ width: "180px" }}>
                                    <MDTypography fontSize="16px" textTransform="capitalize">
                                        {data?.district == null ? "Not Found" : data?.district}
                                    </MDTypography>
                                </Box>
                            ) : data?.location ? (
                                <Box sx={{ width: "180px" }}>
...
export default PartyWorkerPreview